...
In order to be able to evaluate queries with DStream inputs, we only need to implement the algebraic operators for DStream operands.  This is because the \lin{Eval} interface is agnostic to the concrete types of the data it is processing - to evaluate a node it only needs to be able to evaluate its children and be provided an instance of the relevant operator interface to apply to these results. For example, when multiplying two input collections of type \lin{DStream[(K,V)]}, it would first have to be able to evaluate a \lin{LiteralExpr[DStream[(K,V)]]}, which our standard implementation of \lin{LiteralExpr} can do, as it just returns the wrapped value. Then, evaluating the outer \lin{MultiplyExpr} would just require an instance of \lin{Multiply[DStream[(K,V)],DStream[(K,V)],O]}.
