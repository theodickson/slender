...
\subsection{The Ring typeclass} \label{ringtypeclass}
...
An instance of our typeclass \lin{Ring[R]} will constitute evidence that \lin{R} is a valid ring type, and it will contain methods to apply the negate operator to a single value of type \lin{R} as well as a method to add two values of type \lin{R}. It will also contain a definition of the zero value for the ring type, as this is necessary when implementing certain operators. The other operators will be discussed later as they require a separate design pattern. A simplified presentation of \lin{Ring} is as follows:
\vs
\begin{lstlisting}
trait Ring[R] {
  def add(x1: R, x2: R): R
...
For example, we will only generate instances of \lin{Multiply} for two collections with the same key type \lin{K}, but will allow values of any types \lin{R1} and \lin{R2}. By implicitly requiring an instance of \lin{Dot[R1,R2,O]} for some type \lin{O}, we can thus provide the return type.The following instance of \lin{Multiply} for RDDs illustrates this:
\vs\begin{lstlisting}
...
\end{lstlisting}\vs
We now provide some more examples. To sum a \lin{Map[K,R]}, we need an instance of \lin{Ring[R]}, and reduce the values in the map using this instance's \lin{add} method:
\vs\begin{lstlisting}
...
\end{lstlisting}\vs
Note how we use the zero value defined in \lin{Ring} to implement this reduction as a \lin{fold}, thus supporting empty maps.

To compute the dot product of two \lin{Maps}, we require an instance of \lin{Dot} for their values. Then we compute their cartesian product, returning every pair of keys and the dot product of each pair's values:
\begin{lstlisting}
