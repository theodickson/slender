...
class ExecutionTests extends FunSuite {
  ...
  val stringCounts = toExpr(ctx.stringCounts, "stringCounts")

  val stringIntPairs = toExpr(ctx.stringIntPairs, "stringIntPairs")

  val stringIntNestedPairs = toExpr(ctx.stringIntNestedPairs, "stringIntNestedPairs")
