...
object Binder extends BindingImplicits {
  def nonBinder[V <: VariableExpr[V], T, E]: Binder[V,T,E,E] =
...
trait Priority1BindingImplicits extends Priority0BindingImplicits {
  ...
  implicit def ProductKeyBinder[V <: VariableExpr[V],T,C <: HList, CR <: HList]
  (implicit resolve: Binder[V,T,C,CR], trav: ToTraversable.Aux[CR,List,KeyExpr]): Binder[V,T,ProductKeyExpr[C],ProductKeyExpr[CR]] =
    new Binder[V,T,ProductKeyExpr[C],ProductKeyExpr[CR]] {
      def apply(v1: ProductKeyExpr[C]) = ProductKeyExpr(resolve(v1.exprs))
    }

  implicit def ProductRingBinder[V <: VariableExpr[V],T,C <: HList, CR <: HList]
  (implicit resolve: Binder[V,T,C,CR], trav: ToTraversable.Aux[CR,List,RingExpr]): Binder[V,T,ProductRingExpr[C],ProductRingExpr[CR]] =
    new Binder[V,T,ProductRingExpr[C],ProductRingExpr[CR]] {
      def apply(v1: ProductRingExpr[C]) = ProductRingExpr(resolve(v1.exprs))
    }

  implicit def HNilBinder[V <: VariableExpr[V],T]: Binder[V,T,HNil,HNil] = Binder.nonBinder[V,T,HNil]
