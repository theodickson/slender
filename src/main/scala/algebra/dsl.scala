...
object dsl {
  ...
  implicit class ExprImplicits[O,S](e: Expr[O,S]) {

    def +(e1: Expr[O,S])(implicit ring: Ring[O], ringS: Ring[S]) = AddExpr(e,e1)

    def *[O1,S1,O2,S2](e1: Expr[O1,S1])
                      (implicit mult: Multiply[O,O1,O2], multS: Multiply[S,S1,S2]) = MultiplyExpr(e, e1)

    def dot[O1,S1,O2,S2](e1: Expr[O1,S1])
                        (implicit dot: Dot[O,O1,O2], dotS: Dot[S,S1,S2]) = DotExpr(e, e1)

//      def sum[C](implicit coll: Collection[C])
//      def unary_- = Negate(e)
//      def unary_! = Not(e)
//
...
//      def || = this.+ _
  ...
  implicit def toPhysicalCollection[C[_,_],K,R](c: C[K,R])
                                               (implicit coll: Collection[C,K,R]): PhysicalCollection[C,K,R] =
    PhysicalCollection(c)

  implicit class VariableKeyExprOps[K](k: Variable[K]) {

    def <--[C[_,_],KS,R,RS](r: Expr[C[K,R],C[KS,RS]]): (Variable[K], Expr[C[K,R],C[KS,RS]]) = (k,r)

    def ==>[R:Ring,RS:Ring](v: Expr[R,RS]): InfiniteMappingExpr[K,R,RS] = InfiniteMappingExpr(k,v)
  }

  implicit class CollectionExprImplicits[C[_,_],K,R,KS,RS](e: Expr[C[K,R],C[KS,RS]]) {
    def sum(implicit coll: Collection[C, K, R], collS: Collection[C, KS, RS]) = SumExpr[C, K, R, KS, RS](e)
  }

  implicit class StringImplicits(s: String) {
    def tag[T]: Variable[T] = Variable[T](s)
    def ::[T]: Variable[T] = tag[T]
    def <--[C[_,_],K,KS,R,RS](r: Expr[C[K,R],C[KS,RS]]): (Variable[K], Expr[C[K,R],C[KS,RS]]) = (tag[K],r)
  }

  def toK[O:Ring,S:Ring](e: Expr[O,S]): BoxedRingExpr[O,S] = BoxedRingExpr(e)

  def fromK[O:Ring,S:Ring](e: BoxedRingExpr[O,S]): Expr[O,S] = e.c1

  def sng[K,KS](e: Expr[K,KS]): Sng[K,KS,Int,Int] = Sng(e, IntExpr(1))

  def sng[K,KS,R:Ring,RS:Ring](k: Expr[K,KS], r: Expr[R,RS]): Sng[K,KS,R,RS] = Sng(k, r)
...
//  case class ForComprehensionBuilder2[C[_,_],K,KS,R,RS,V <: DSLVariable](x: V, r1)
  ...
  case class ForComprehensionBuilder[C[_,_],K,KS,R,RS](x: Variable[K], r1: Expr[C[K,R],C[KS,RS]]) {

    def Collect[R1,RS1,R2,RS2]
               (r2: Expr[R1,RS1])
               (implicit mult: Multiply[C[K,R],K=>R1,C[K,R2]], multS: Multiply[C[KS,RS],K=>RS1,C[KS,RS2]],
                coll: Collection[C,K,R2], collS: Collection[C,KS,RS2]) = {
      val multiplied = MultiplyExpr(r1,InfiniteMappingExpr(x,r2))
      SumExpr[C,K,R2,KS,RS2](multiplied)(coll,collS)
    }
...
//    def Yield(k: KeyExpr): RingExpr = Collect(sng(k))
  ...
  //
    object For {
      def apply[C[_,_],K,KS,R,RS](paired: (Variable[K], Expr[C[K,R],C[KS,RS]])): ForComprehensionBuilder[C,K,KS,R,RS] =
        ForComprehensionBuilder[C,K,KS,R,RS](paired._1, paired._2)
    }
  //
