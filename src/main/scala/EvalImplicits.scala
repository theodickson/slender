...
trait EvalImplicits {
  ...
  implicit def InfiniteMappingEval[V <: VariableExpr[V] { type Type = KT },R <: Expr[R],KT,RT]
  ...
  implicit def MultiplyEval[E1 <: Expr[E1],E2 <: Expr[E2],T1,T2,O](implicit eval1: Eval[E1,T1],
                                                                   eval2: Eval[E2,T2],
  ...
  implicit def DotEval[E1 <: Expr[E1],E2 <: Expr[E2],T1,T2,O](implicit eval1: Eval[E1,T1],
                                                              eval2: Eval[E2,T2],
  ...
  implicit def AddEval[E1 <: Expr[E1],E2 <: Expr[E2],T](implicit eval1: Eval[E1,T],
                                                        eval2: Eval[E2,T],
  ...
  implicit def SumEval[E <: Expr[E],C[_,_],K,R,O](implicit eval: Eval[E,C[K,R]], coll: Collection[C,K,R]):
  Eval[SumExpr[E],R] = new Eval[SumExpr[E],R] {
    def apply(v1: SumExpr[E], v2: BoundVars): R = coll.sum(eval(v1.c1,v2))
  }

//  implicit def NotEval[E <: Expr[E],T](implicit eval: Eval[E,T], ring: Ring[T]): Eval[NotExpr[E],T] = new Eval[NotExpr[E],T] {
//    def apply(v1: NotExpr[E], v2: BoundVars): T = ring.not(eval(v1.c1,v2))
//  }
//
//  implicit def NegateEval[E <: Expr[E],T](implicit eval: Eval[E,T], ring: Ring[T]): Eval[NegateExpr[E],T] = new Eval[NegateExpr[E],T] {
//    def apply(v1: NegateExpr[E], v2: BoundVars): T = ring.negate(eval(v1.c1,v2))
//  }

  implicit def SngEval[K <: Expr[K],R <: Expr[R],TK,TR](implicit evalK: Eval[K,TK], evalR: Eval[R,TR]):
